...
class CHS_ZipReader
{
    ...
    protected function releaseLock()
    {
        if (file_exists($this->lock_file)) {
            ...
            CHS_Logger::logs("Release requested but lock not found: {$this->lock_file}");
        }
    }
    ...
    public function process_all_files()
    {
        ...
        if (!$this->acquireLock()) {
            CHS_Logger::logs("Lock active - another run in progress, skipping.");
            return $results;
        }
        ...
        try {
            ...
            CHS_Logger::logs("Error in process_all_files: " . $e->getMessage());
        } finally {
            $this->releaseLock();
        }
    ...
    protected function process_single_zip($zip_file)
    {
        ...
        try {
            ...
            CHS_Logger::logs("Error processing ZIP " . basename($zip_file) . ": " . $e->getMessage());
            $this->move_to_failed($zip_file);
            return null;
        }
    }
    ...
    protected function scan_and_log_files($run_dir)
    {
        ...
        try {
            ...
            CHS_Logger::logs("Error scanning run_dir {$run_dir}: " . $e->getMessage());
        }
    ...
    protected function generic_process_files($run_dir, $run_id, $sourceFile = null)
    {
        ...
        try {
            ...
            CHS_Logger::logs("Error in generic_process_files for {$run_dir}: " . $e->getMessage());
        }
    ...
    protected function parse_generic_csv($file, $delimiter = ',', $sourceFile = null)
    {
        ...
        try {
            ...
            while (($data = fgetcsv($handle, 0, $delimiter)) !== false) {
                ...
                if (strtoupper(basename($file)) === 'PHOTOS.TXT') {
                    $media_data_array[] = $data;
                }

                $mapped_data = CHS_Centris_Mapping::getMappedData($data, strtoupper(basename($file)));
                $_centris_mls = $mapped_data['_centris_mls'] ?? null;
                if (! $_centris_mls) {
                    // skip this row if no _centris_mls found
                    continue;
                }

                // initialize array for this MLS if not exists
                if (! isset($file_data_array[$_centris_mls])) {
                    $file_data_array[$_centris_mls] = [];
                }

                // now loop through mapped data safely
                foreach ($mapped_data as $key => $value) {
                    if ($key === '' || $key === null) continue;

                    // skip _centris_mls, it's already used
                    if ($key === '_centris_mls' && isset($file_data_array[$_centris_mls][$key]) && $file_data_array[$_centris_mls][$key] === $value) {
                        continue;
                    }
                    // handle multiple values
                    if (isset($file_data_array[$_centris_mls][$key])) {
                        if (is_array($file_data_array[$_centris_mls][$key])) {
                            if (!in_array($value, $file_data_array[$_centris_mls][$key])) {
                                $file_data_array[$_centris_mls][$key][] = $value;
                            }
                        } else {
                            if ($file_data_array[$_centris_mls][$key] != $value) {
                                $existing_value = $file_data_array[$_centris_mls][$key];
                                $file_data_array[$_centris_mls][$key] = [$existing_value, $value];
                            }
                        }
                    } else {
                        $file_data_array[$_centris_mls][$key] = $value;
                ...
                // CHS_Logger::log("parsing data " . basename($file) . ": " . json_encode($mapped_data));
            }
            ...
            CHS_Logger::logs("Error parsing file " . basename($file) . ": " . $e->getMessage());
        }
    ...
    protected function archive_zip($zip_file)
    {
        try {
            ...
            CHS_Logger::logs("Error archiving " . basename($zip_file) . ": " . $e->getMessage());
        }
    }
    ...
    protected function archive_specific_file($file)
    {
        try {
            ...
            CHS_Logger::logs("Error archiving file " . basename($file) . ": " . $e->getMessage());
        }
    }
    ...
    protected function move_to_failed($file)
    {
        try {
            ...
            CHS_Logger::logs("Error moving to failed " . basename($file) . ": " . $e->getMessage());
        }
    }
